...
if (isset($_POST['TPago']) && !isset($_POST['TAbono'])) {
    ...
    if ($pago >= $total) {
        ...
        swal({
          ...
          function(isConfirm) {
            if (isConfirm) {
                window.location.href='finalizarPago.php?total=$total&pago=$pago&cambio=$cambio'
            } else {
        ...
        echo "<script> swal({title:'Atención',text:'Ingrese una cantidad de dinero válida',type:'warning'});</script>";
    }
...
if (isset($_POST['TAbono']) && isset($_POST['TPago'])) {
    ...
    swal({
      ...
      function(isConfirm) {
        if (isConfirm) {
            window.location.href='finalizarPago.php?total=$total&pago=$pago&cambio=$cambio&totald=$total_deuda&abono=$abono'
        } else {
            window.location.href='VVentas.php'
        }
      });
