...
$consulta_usuariocafi = "INSERT INTO usuarioscafi (email,acceso,entrada_sistema,contrasena) VALUES (?,?,?,?)";
$tipo_datos_usuariocafi = "ssss";
...
echo $conexion->consultaPreparada($datos_usuariocafi,$consulta_usuariocafi,1,$tipo_datos_usuariocafi);
  ...
  $editar= "UPDATE persona INNER JOIN usuarioscafi ON persona.email=usuariosab.email SET rfc= ?, nombre = ?, cp = ?, calle_numero = ?, colonia = ?, localidad = ?, municipio = ?, 
  ...
  echo $conexion->consultaPreparada($datos_usuariocafi,$editar,1,$tipo_datos);
    ...
    sexo,entrada_sistema,contrasena FROM persona INNER JOIN usuarioscafi ON persona.email=usuariosab.email WHERE acceso = ?";
    $datos = array();
